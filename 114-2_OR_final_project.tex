...
Specifically, Constraint (2) utilizes the Big-M method along with a strictly positive tolerance ($\epsilon$) to link the binary winning variable $w_{k}$ with the scoring performance.
This ensures that $w_{k}$ is assigned a value of 1 only if the total expected runs allowed by the assigned pitchers are strictly less than the team's offensive runs $C_{k}$.
...
In continuous mathematical programming, standard solvers require closed sets (i.e., $\le$) and cannot naturally process strict inequalities ($<$).
To strictly enforce the baseball rule that a win requires fewer runs allowed than scored ($\sum E_{ij}x_{ijk} < C_k$), we introduce a marginally small positive tolerance, $\epsilon$.
...
Consequently, the formulated constraint $\sum E_{ij}x_{ijk} - C_{k} + \epsilon \le M(1-w_{k})$ intentionally assigns $w_k = 0$ in the event of an exact mathematical tie ($\sum E_{ij}x_{ijk} = C_{k}$).
We acknowledge that the inability to naturally represent a tie is an inherent structural limitation of the model, arising from the approximation of discrete game outcomes via continuous expected metrics.
However, while this forced classification is undeniably conservative, it explicitly aligns with the core philosophy of our modeling scenario.

In our framework, the offensive run production ($C_k$) is treated as a predetermined, fixed parameter.
Therefore, the sole responsibility for securing a victory relies entirely on the efficiency of the pitching deployment.
If a pitching assignment merely allows as many runs as the offense provides, it has failed its primary defensive objective to preserve a lead.
By utilizing $\epsilon$ to classify a projected tie as a non-win, the model accurately penalizes assignments that fail to definitively secure the game, reinforcing the expectation that the pitching staff must strictly outperform the projected run support.

\textbf{Calibration of Big-M ($M$):}

To properly enforce the logical implication in Constraint (2) using the Big-M method, the parameter $M$ must serve as a valid upper bound for the newly formulated left-hand side of the inequality: $\sum_{i\in I}\sum_{j\in J} E_{ij}x_{ijk} - C_{k} + \epsilon$.

A mathematically rigorous upper bound must evaluate the extreme values of all terms independently.
...
Consequently, incorporating the tolerance, a theoretically safe upper bound for the entire expression is derived as $\sum_{i,j} E_{ij} - 0 + \epsilon$.
...
Grounded in this worst-case scenario, setting $M$ to a constant such as 40 or 50 provides a sufficient buffer to safely encompass any realistic baseball anomaly—including the marginal addition of $\epsilon$—while preserving a tight feasible region for optimal solver performance.
